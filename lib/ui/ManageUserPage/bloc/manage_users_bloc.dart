...
class ManageUsersBloc extends Bloc<ManageUsersEvent, ManageUsersState> {
  ...
  Future<void> _onLoadUsers(LoadUsers event, Emitter<ManageUsersState> emit) async {
    emit(state.copyWith(isLoading: true, currentAction: 'loading_users', isProcessing: true));
    
    try {
      final users = await _userRepository.getUsers();
      emit(ManageUsersState.success(users: users, message: 'Users loaded successfully'));
    } catch (e) {
      emit(ManageUsersState.failure('Failed to load users: ${e.toString()}', action: 'load_users'));
  ...
  Future<void> _onAddUser(AddUser event, Emitter<ManageUsersState> emit) async {
    ...
    try {
      ...
      final users = await _userRepository.getUsers();
      emit(ManageUsersState.success(
        users: users,
        message: 'User added successfully',
      ));
    } catch (e) {
      emit(ManageUsersState.failure(
  ...
  Future<void> _onUpdateUser(UpdateUser event, Emitter<ManageUsersState> emit) async {
  ...
  Future<void> _onDeleteUser(DeleteUser event, Emitter<ManageUsersState> emit) async {
